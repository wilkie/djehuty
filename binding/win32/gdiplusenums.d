 ...
 * gdiplusenums.d
 *
 * This module implements GdiPlusEnums.h for D. The original copyright
 * info is given below.
 *
...
module binding.win32.gdiplusenums;

import binding.win32.wingdi;

// The original copyright notice from GdiPlusEnums.h from which this module is based.
...
bool ObjectTypeIsValue(ObjectType type) {
	return ((type >= ObjectType.ObjectTypeMin) && (type <= ObjectType.ObjectTypeMax));
}
...
const auto GDIP_EMFPLUS_RECORD_BASE        = 0x00004000;
const auto GDIP_WMF_RECORD_BASE            = 0x00010000;

template GDIP_WMF_RECORD_TO_EMFPLUS(uint n) {
	const uint GDIP_WMF_RECORD_TO_EMFPLUS = ((n) | GDIP_WMF_RECORD_BASE);
}
...
uint GDIP_EMFPLUS_RECORD_TO_WMF(EmfPlusRecordType n) {
	return ((n) & (~GDIP_WMF_RECORD_BASE));
}

bool GDIP_IS_WMF_RECORDTYPE(uint n) {
